...
from transformers.data.processors.squad import SquadResult, SquadV1Processor, SquadV2Processor, read_squad_examples
...
from examples.legacy.query_strategies import RandomSampling, LeastConfidence, MarginSampling, EntropySampling, Beyondrc,\
...
def write_jsonl(passages, output_path):
...
def train(args, train_dataset, model, tokenizer):
    ...
    print("train_batch {} n_gpu {}".format(args.train_batch_size, args.n_gpu), flush=True)
    ...
    # Added here for re-productivity
    set_seed(args)
    param_iter = 1
...
def evaluate(args, model, tokenizer, prefix="", active=False, eval_dataset=None, output_type='prob', n_dropout=3):
    ...
    if active:
        if output_type == 'prob':
            ...
            for batch_id, batch in tqdm(enumerate(eval_dataloader), desc="Calculating Probability"):
                model.eval()
                batch = tuple(t.to(args.device) for t in batch)
                ...
                with torch.no_grad():
                    ...
                    if args.model_type in ["xlnet", "xlm"]:
                        ...
                        if hasattr(model, "config") and hasattr(model.config, "lang2id"):
                            inputs.update(
                                {"langs": (torch.ones(batch[0].shape, dtype=torch.int64) * args.lang_id).to(
                                    args.device)}
                            )
            ...
            probs = torch.zeros([n_dropout, len(dataset), args.n_best_size])
        ...
        if args.dataset == 'squad':
            if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.train_file)):
                try:
                    ...
                    raise ImportError("If not data_dir is specified, tensorflow_dataset needs to be installed.")

                if args.version_2_with_negative:
                    logger.warn("tensorflow_dataset does not handle version 2 of SQuAD.")
            ...
            if evaluate:
                examples = read_squad_examples(args.predict_file, 'eval')
            else:
                examples = read_squad_examples(args.train_file, 'train')
...
def load_checkpoint(args):
    ...
    model = AutoModelForQuestionAnswering.from_pretrained(args.checkpoint)  # , force_download=True)
...
def active_learning(args, dataset, model, tokenizer):
    NUM_INIT_LB = args.init #10000
    NUM_QUERY = args.query  #10000 #2000
    # load dataset
    if args.flag_sub==0:
        dataset_tr = dataset  # Subset(dataset, range(0, 10000))
    else:
    ...
    logger.info('number of active learning rounds: {}'.format(NUM_ROUND))
    ...
    # model, tokenizer = load_checkpoint(args)
    results = evaluate(args, model, tokenizer)
...
#     dup_index=[]
    ...
    for rd in range(1, NUM_ROUND + 1):
        ...
        strategy.T = rd
        # query
        # model, tokenizer = load_checkpoint(args)  # use checkpoint for sample selection
#         q_idxs, rddup_ind = strategy.query(NUM_QUERY, model, tokenizer)
        if args.al_method == "beyond":
            q_idxs,rddup_ind = strategy.query(NUM_QUERY, model, tokenizer)
            dup_index = rddup_ind
            idxs_lb[q_idxs] = True
            ...
            q_idxs = strategy.query(NUM_QUERY, model, tokenizer)
            idxs_lb[q_idxs] = True
...
#             global_step, tr_loss = strategy.train(model, tokenizer)
    ...
    write_jsonl(metrics, os.path.join(args.output_dir, args.al_method + '_' + incremental_string + '_' +
                                      str(NUM_INIT_LB) + '_' + str(NUM_QUERY) + '_' + str(NUM_ROUND) + '_' + str(n_pool)
                                      + '_' + str(int(args.num_train_epochs)) + '_v'+str(args.version)+'_epoch.jsonl'))
...
def main():
    ...
    parser.add_argument("--do_active", action="store_true", help="Whether to run active learning.")
    parser.add_argument("--al_method", default='random', type=str, help="Which active learning method to use.")
    parser.add_argument("--al_incremental", action="store_true",
                        help="Whether to update model incrementally during active learning.")
    parser.add_argument("--dataset", type=str, default="squad", help="name of the dataset")
    parser.add_argument("--init", type=int, default=10000, help="initial")
    ...
    parser.add_argument("--query_learnrate", default=1.1, type=float, help="The initial learning rate for Beyond")
    parser.add_argument(
    ...
    parser.add_argument("--lambda_parameter", default=100, type=float, help="loss param L2")
    ...
    parser.add_argument("--flag_sub", type=int, default="", help="subset.")
    parser.add_argument("--num_round", type=int, default="0", help="beyond.")
    parser.add_argument("--thresh", type=float, default = "0.01", help="beyond.")
    parser.add_argument("--threads", type=int, default=64, help="multiple threads for converting example to features")
    args = parser.parse_args()
    
    log_name = os.path.join(args.output_dir, str(args.lambda_parameter)+'_'+str(args.version) + '_'+args.al_method + '_' + args.model_name_or_path+"_i"+str(args.init)+ "_q"+str(args.query)+str(args.thresh) + "_" + str(args.num_round)+'_' + str(args.learning_rate)+'_sub'+str(args.flag_sub)+'_weight_'+str(args.weight_decay))
#     log_name = os.path.join(args.output_dir, str(args.version) +  args.dataset +'_gpu_'+str(args.n_gpu))
    logger.info("Training/evaluation parameters %s", args)
    fsock = open(log_name +'_run.log', 'w', 1)
    sys.stdout = fsock
    print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), flush=True)
#     file_name = os.path.join(args.output_dir, args.al_method +'_epoch.jsonl') # do_active

    file_name = os.path.join(args.output_dir, args.dataset +'_gpu_'+str(args.n_gpu)+'.jsonl') # do_train
    ...
    # Before we do anything with models,
    # we want to ensure that we get fp16 execution of torch.einsum if args.fp16 is set.
    ...
    if args.do_train:
        ...
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)
        print("global_step = {}, average loss = {}".format(global_step, tr_loss), flush=True )
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        print("Saving model checkpoint to {}".format(args.output_dir), flush=True )
